<html>
	<head>
		...
		<script language="JavaScript">
			...
			function EBDocumentComplete() {
				...
				$.getJSON('../../php/index.php', function(json) 
				{
					...
					if(typeof(json['rss']['channel']['item']) == 'undefined')
					{
						//$('a').html('nothing to show...');
						//$('a').attr('src', '');
						$('.headerTxt').css('background', 'url(../../images/btn_grey.png) 0 0');
						$('.headerTxt').css('cursor', 'pointer');
						$('.headerTxt').click(function(){
							NavigateInMainFrame('http://bild.de');
							setTimeout('CloseFloatingWindow()', 400);
						});
						$('.container').hide();
						ChangeWidth(111);
					}
					// case the feed has data
					else{
							ChangeWidth(427);
							$('.headerTxt').css('background', 'url(../../images/btn_red.png) 0 0');
							$('.headerTxt').css('cursor', 'default');
							$('.container').show();
							$('a').html(getValueByPath(json,getPathbyDivName("title")));
							$('a').attr('href', getValueByPath(json,getPathbyDivName("link")));
					}
			...
			$(document).ready(function() {
				interval = setInterval('EBDocumentComplete()', 30*1000);
		...
		<style>
			...
			.headerTxt{float: left; background:url(../../images/btn_red.png) 0 0; width: 111px; height: 22px; margin-top:2px;}
			.feedText{float: left;}
			.container a { color: white;font-family: arial; font-size: 15px; font-weight: bold; line-height: 22px;}
			.container { background:url(../../images/btn_red.png) -111px 0; width: 313px; height: 22px; margin-top:2px;}
		</style>
	</head>
	<body>
		<div class="headerTxt"></div>
		<div class="container feedText" id="scroller_container">
			<div id="scroller">
				<a href="" title=""></a>
			</div>
